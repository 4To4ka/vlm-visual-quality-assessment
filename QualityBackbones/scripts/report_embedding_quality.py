...
def _default_run_dir(outputs_root: Path) -> Path:
    ...
    return outputs_root / "embedding_quality_runs" / stamp


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Exact embedding-quality report for IQA: pairwise embedding distance vs pairwise score distance "
            "correlations per dataset/model/layer"
        )
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=("abs",),
        help="Score distances: abs, sq (comma or space separated)",
    )
    parser.add_argument(
        "--corr-metrics",
        nargs="*",
        default=("pcc,scc,kcc",),
        help="Correlation metrics: pcc, scc, kcc (comma or space separated)",
    )

    parser.add_argument("--block-size", type=int, default=512)
    parser.add_argument("--jobs", type=str, default="auto", help="Worker processes count or 'auto'")
    parser.add_argument("--tmp-dir", type=Path, default=Path("outputs/embedding_quality_cache"))
...
def _print_report(report: dict, print_limit: int) -> None:
    rows: list[dict] = report.get("results", [])
    dataset_summaries: list[dict] = report.get("dataset_summaries", [])
    ...
    print(f"session_elapsed_sec: {report.get('session_elapsed_sec', float('nan')):.3f}")
    ...
    if dataset_summaries:
        ...
        for item in dataset_summaries:
            print(
                ...
                f"elapsed_sec={float(item.get('elapsed_sec', 0.0)):.3f}"
            )
        print()

    best_layers = report.get("best_layers", [])
    if best_layers:
        print(f"Best layers (first {max(1, print_limit)}):")
        for row in best_layers[: max(1, print_limit)]:
            print(
                "  - "
                f"{row['dataset']} / {row['model_key']} / {row['corr_metric']} / "
                f"{row['embedding_distance']} / {row['score_distance']}: "
                f"layer={row['layer_name']} ({row['layer_index']}) value={row['value']:.6f}"
            )
        print()
...
def main() -> None:
    ...
    score_distances = parse_metric_list(args.score_distance, kind="score")
    ...
    if embedding_distances is None or score_distances is None or corr_metrics is None:
